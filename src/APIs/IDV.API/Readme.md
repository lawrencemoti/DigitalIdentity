...
WORKDIR /src
COPY ["IDV.API.csproj", "IDV.API/"]
RUN dotnet restore "IDV.API/IDV.API.csproj"

WORKDIR "/src/IDV.API"
COPY . .
RUN dotnet build "IDV.API.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "IDV.API.csproj" \
...
Execute unit tests
```
    cd "IDV.API/test/IDV.API.Tests"
    dotnet test
```

Deploy application
```
    cd "IDV.API/src/IDV.API"
